...
let hasLoaded = false; //has the window loaded yet?
...
function loadCards() {
  ...
  if (window.innerWidth < 768) {
    createStaffMember(1);
    hasLoaded = false;
    console.log("loaded on mobile, should see 2 people");
    return;
  } else {
    createStaffMember(staff.length);
    document.querySelector(".staff-inside").classList.toggle("active");
    hasLoaded = false;
    console.log("loaded on desktop, should see all people");
    return;
  }
}
